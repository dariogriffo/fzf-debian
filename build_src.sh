...
if [ -z "$fzf_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <fzf_version> <build_version>"
    echo "Example: $0 0.62.0 1"
    exit 1
fi

PACKAGE_NAME="fzf"
ORIG_TARBALL="${PACKAGE_NAME}_${fzf_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${fzf_VERSION}"

echo "Creating Debian/Ubuntu source packages for fzf ${fzf_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/junegunn/fzf/archive/refs/tags/v${fzf_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${fzf_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as fzf-0.x.y/ which matches our BUILD_DIR
...
fzf (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${fzf_VERSION}.
